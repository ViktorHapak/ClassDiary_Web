...
export class UserDetailsComponent {
  ...
  getUser(id:any){
    this.userService.get(this.index).subscribe({
      next: (data) => {
        ...
        username: data.username, birth: data.birth, password: data.password, userrole: data.userrole,
  ...
  protected async getStudents() : Promise<void> {
    try{
      let params =
      ...
      let searchData = await firstValueFrom(this.studentService.getAllByParams(params));

      // ⏳ Delayed resolution handling
      setTimeout(() => {
        const { students, totalItems }: any = searchData;
        this.studentNames = students.map(student => student.name)
          .filter(studentName => !this.user.children.includes(studentName));
        if (this.searchStudentRequest == "") this.fetchedStudents = false
        else this.fetchedStudents = true;
      }, 100);
  ...
  isAuthenticated():boolean{
    if(this.storageService.getUser().username == this.user.username) return true;
    else return false
  }
}
